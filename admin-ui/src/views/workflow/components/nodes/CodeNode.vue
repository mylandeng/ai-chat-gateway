<template>
  <div class="wf-node code-node" :class="{ selected }">
    <Handle type="target" :position="Position.Left" />
    <div class="node-header">
      <span class="node-icon">💻</span>
      <span class="node-label">{{ data.label || '代码' }}</span>
    </div>
    <div class="node-body">
      <div class="node-info" v-if="data.code">
        <code>{{ data.code.substring(0, 30) }}{{ data.code.length > 30 ? '...' : '' }}</code>
      </div>
      <div class="node-info text-muted" v-else>点击编写代码</div>
    </div>
...
.code-node .node-header { background: rgba(250,140,22,0.15); }
.node-icon { font-size: 16px; }
.node-label { font-weight: 600; font-family: var(--nx-font-mono); font-size: 12px; }
.node-body { padding: 6px 12px; }
.node-info { font-size: 11px; opacity: 0.8; }
.node-info code { font-family: var(--nx-font-mono); background: rgba(255,255,255,0.05); padding: 2px 6px; border-radius: 4px; }
