<template>
  <div class="wf-node end-node" :class="{ selected }">
    <Handle type="target" :position="Position.Left" />
    <div class="node-header">
      <span class="node-icon">⏹</span>
      <span class="node-label">{{ data.label || '结束' }}</span>
    </div>
    <div class="node-body">
      <span class="node-hint">工作流终点</span>
    </div>
...
<style scoped>
.wf-node { min-width: 140px; border-radius: 6px; border: 2px solid transparent; background: var(--nx-bg-surface, #1a1a2e); color: var(--nx-text-primary, #e0e0e0); font-size: 13px; box-shadow: 0 2px 8px rgba(0,0,0,0.3); }
...
.end-node .node-header { background: rgba(255,77,79,0.15); }
...
.node-status { padding: 4px 12px 6px; font-family: var(--nx-font-mono); font-size: 10px; letter-spacing: 1px; text-transform: uppercase; }
