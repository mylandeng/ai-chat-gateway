<template>
  <div class="wf-node http-node" :class="{ selected }">
    <Handle type="target" :position="Position.Left" />
    <div class="node-header">
      <span class="node-icon">🌐</span>
      <span class="node-label">{{ data.label || 'HTTP' }}</span>
    </div>
    <div class="node-body">
      <div class="node-info" v-if="data.method && data.url">
        <span class="http-method">{{ data.method }}</span> {{ truncUrl }}
      </div>
      <div class="node-info text-muted" v-else>点击配置请求</div>
    </div>
...
.http-node .node-header { background: rgba(235,47,150,0.15); }
.node-icon { font-size: 16px; }
.node-label { font-weight: 600; font-family: var(--nx-font-mono); font-size: 12px; }
.node-body { padding: 6px 12px; }
.node-info { font-size: 11px; opacity: 0.8; font-family: var(--nx-font-mono); }
.http-method { font-weight: 700; color: var(--nx-accent-amber); }
