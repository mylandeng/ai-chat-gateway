<template>
  <div class="wf-node tool-node" :class="{ selected }">
    <Handle type="target" :position="Position.Left" />
    <div class="node-header">
      <span class="node-icon">🔧</span>
      <span class="node-label">{{ data.label || '工具' }}</span>
    </div>
    <div class="node-body">
      <div class="node-info" v-if="data.toolName">{{ data.toolName }}</div>
      <div class="node-info text-muted" v-else>点击配置工具</div>
    </div>
...
.tool-node .node-header { background: rgba(250,173,20,0.15); }
