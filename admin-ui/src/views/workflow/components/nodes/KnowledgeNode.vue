<template>
  <div class="wf-node knowledge-node" :class="{ selected }">
    <Handle type="target" :position="Position.Left" />
    <div class="node-header">
      <span class="node-icon">📚</span>
      <span class="node-label">{{ data.label || '知识库' }}</span>
    </div>
    <div class="node-body">
      <div class="node-info" v-if="data.knowledgeBaseId">KB#{{ data.knowledgeBaseId }}</div>
      <div class="node-info text-muted" v-else>点击配置知识库</div>
    </div>
...
.knowledge-node .node-header { background: rgba(82,196,26,0.15); }
.node-icon { font-size: 16px; }
.node-label { font-weight: 600; font-family: var(--nx-font-mono); font-size: 12px; }
.node-body { padding: 6px 12px; }
.node-info { font-size: 12px; opacity: 0.8; font-family: var(--nx-font-mono); }
